...
class fileDB extends Component
{
    ...
    public function insert($array)
    {
        ...
        if ($this->hasEventHandles('afterInsert')) {
...
abstract class Behavior  //行为的抽象类
{
    ...
    public function attach($to) //和某个类连接
    {
        ...
        //连接的时候，把所以的On事件函数 绑定起来
        $methods = get_class_methods($this);
        foreach ($methods as $methodsName) {
            if (strpos($methodsName, 'on') === 0 && strlen($methodsName) > 2) {
                $this->owner->on(lcfirst(substr($methodsName, 2)), [$this, $methodsName]);
            }
        }
...
echo $fileDb->insert(['langlilangligelang', 'sddddd']);
echo $fileDb->getFirstRecord(); //这样在 fileDb 使用行为里的函数了。可以在DbBehavior都直接扩展方法了。


//总结： 这样 可以把 处理fileDB 类时，可以直接在不同行为中扩展方法。也可以获取不同的行为
//      实现对象时，可以根据不同的需求连接不同的行为。并且绑定相应的事件。